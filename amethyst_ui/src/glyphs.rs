                        ...
                        texts_not_hidden_query_with_optional_editing.for_each_mut(
                            ...
                            |(entity, transform, ui_text, editing, tint)| {
                                    ...
                                    (font_lookup.id(), font_asset)
                                {
                                    ...
                                    let next_z =
                                        if let Some(val) = self.glyph_entity_cache.keys().last() {
                                            val + 1
                                        } else {
                                            0
                                        };
                                    ...
                                    texts_not_hidden_query_with_optional_editing.for_each_mut(
                                        ...
                                        |(entity, transform, ui_text, editing, tint)| {
                                            let len = vertices[glyph_ctr..]
                                                ...
                                                .count();
                                            let entity_verts = vertices[glyph_ctr..glyph_ctr + len]
                                                .iter()
                                                .map(|v| v.1);
                                            glyph_ctr += len;
                                            ...
                                            if let Some(editing) = editing {
                                                let font = font_storage.get(&ui_text.font).expect(
                                                    "Font with rendered glyphs must be loaded",
                                                );
                                                let scale = Scale::uniform(ui_text.font_size);
                                                let v_metrics = font.0.v_metrics(scale);
                                                let height = v_metrics.ascent - v_metrics.descent;
                                                let offset =
                                                    (v_metrics.ascent + v_metrics.descent) * 0.5;
                                                ...
                                                let iter = ui_text.cached_glyphs[start..end]
                                                    .iter()
                                                    .map(|g| UiArgs {
                                                        coords: [
                                                        ...
                                                        .into(),
                                                        dimensions: [g.advance_width, height]
                                                            .into(),
