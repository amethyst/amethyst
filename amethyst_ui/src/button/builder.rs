    ...
    pub fn build(mut self, mut res: UiButtonBuilderResources) -> Entity {
        ...
        if self.hover_image.is_some() || self.press_image.is_some() {
            res.action_image
                .insert(
                    image_entity,
                    OnUiActionImage::new(image_handle, self.hover_image, self.press_image),
                )
                .unwrap();
        }
        
        if self.hover_sound.is_some() || self.press_sound.is_some() {
            res.action_sound
                .insert(
                    image_entity,
                    OnUiActionSound::new(self.hover_sound, self.press_sound),
                )
                .unwrap();
        }
        image_entity
