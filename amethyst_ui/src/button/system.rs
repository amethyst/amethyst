    ...
    fn run(
        ...
        for event in events.read(event_reader) {
            ...
            match event.event_type {
                ClickStart => {
                    if let Some(texture) = action_image.and_then(|i| i.press_image.as_ref()) {
                        ...
                        if let Some(action_image) = action_image {
                            if Some(event.target) == self.hovered {
                                if let Some(hover_texture) = action_image.hover_image.as_ref() {
                                    ...
                                    image.texture = action_image.normal_image.clone();
                                }
                            } else {
                                image.texture = action_image.normal_image.clone();
                            }
                ...
                HoverStop => {
                    self.hovered = None;
                    if let (Some(image), Some(action_image)) = (image, action_image) {
                        image.texture = action_image.normal_image.clone();
                    }
                    if let Some(button) = button {
                        for &child in hierarchy.children(event.target).unwrap_or(&[]) {
