    ...
    pub fn build(&mut self) -> Box<dyn ParallelRunnable> {
        Box::new(
            SystemBuilder::new("SelectionKeyboardSystem")
                ...
                .build(move |commands, world, (window_events, cached,ui_events),
                             selected_query| {
                    /*
                           Algorithm in use:
                    ...
                    for event in window_events.read(&mut self.window_reader_id) {
                        ...
                        } = *event
                        {
                            ...
                            let highest = cached.highest_order_selected_index(selected_query.iter(world));
                            ...
                            if let Some(highest) = highest {
                                ...
                                // Select Replace
                                selected_query.for_each(world, |entity, _| {
        ...
        Box::new(
            SystemBuilder::new("SelectionMouseSystem")
                ...
                .build(move |commands, world, (cached, input_handler, ui_events),
                    ...
                    let (mut selectable_subworld, mut selected_subworld) = world.split_for_query(selectables);
                    ...
                    for ev in ui_events.read(&mut self.ui_reader_id) {
                        if let UiEventType::ClickStart = ev.event_type {
                            if selectables.get(&selectable_subworld, ev.target).is_err() {
                                selected_query.for_each_mut(&selected_subworld, |(entity, _)| {
                            ...
                            let highest = cached.highest_order_selected_index(selected_query.iter(&selected_subworld));
                            ...
                            if let Some(highest) = highest {
                                ...
                                if highest_is_select {
                                    if shift {
                                        ...
                                        selected_query.for_each(&selected_subworld, |(entity, _)| {
                                            commands.remove_component::<Selected>(entity);
                                        });
                                        commands.add_component(clicked, Selected);
                                        emitted.push(UiEvent::new(UiEventType::Focus, clicked));
