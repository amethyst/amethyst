...
impl EventsPump {
    pub fn pump_events(&mut self, event_handler: &mut EventChannel<Event>) {
...
impl<'a> System<'a> for EventsPump {
    type SystemData = FetchMut<'a, EventChannel<Event>>;

    fn run(&mut self, mut event_handler: FetchMut<'a, EventChannel<Event>>) {
        self.pump_events(&mut event_handler);
    }
}
