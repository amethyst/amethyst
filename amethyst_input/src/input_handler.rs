    ...
    AC: Hash + Eq + Clone + Send + Sync + 'static,
{
    ...
    pub fn controller_buttons_that_are_down(&self) -> ControllerButtons {
    ...
    pub fn connected_controllers(&self) -> Controllers {
        self.connected_controllers
            .iter()
            .map((|ids| ids.0) as fn(&(u32, u32)) -> u32)
    }
    ...
    pub fn buttons_that_are_down<'a>(&self) -> Buttons {
        ...
        let controller_buttons = self
            .pressed_controller_buttons
            .iter()
            .map((|&gb| Button::Controller(gb.0, gb.1)) as fn(&(u32, ControllerButton)) -> Button);
        Buttons {
            iterator: mouse_buttons.chain(keys).chain(controller_buttons),
        }
    }
