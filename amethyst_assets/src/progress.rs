...
impl Tracker for ProgressCounterTracker {
    ...
    fn fail(self: Box<Self>, handle_id: u32, asset_type_name: &'static str, asset_name: String, e: Error) {
...
impl Tracker for () {
    fn success(self: Box<Self>) {}
    fn fail(self: Box<Self>, handle_id: u32, asset_type_name: &'static str, asset_name: String, e: Error) {
        error!("error loading handle {}, {}, with name {}, caused by: {:?}", handle_id, asset_type_name, asset_name, e);
