...
use store::AnyStore;
use {Allocator, Asset, BoxedErr, Directory, Format, AssetError, LoadError, Store};
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        match Arc::try_unwrap(self.inner.take().unwrap()) {
            Ok(x) => {
                ...
                unsafe {
                    x.value.into_inner().expect("Thread panicked").map(
                        Async::Ready,
                    )
                }
            }
...
impl Loader {
    ...
    where
        ...
        String: Borrow<S>,
    {
        let context = self.context::<A>();

        reload_asset_future::<A, F, N, _>(
            context.clone(),
            format,
            name,
            self.store(store),
    ...
    fn context<A>(&self) -> &Arc<A::Context>
    where
        A: Asset + 'static,
    {
        let context = self.contexts.get(&TypeId::of::<A>()).expect(
...
pub fn load_asset_future<A, F, N, S>(
