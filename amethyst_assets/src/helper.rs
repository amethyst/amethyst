...
use amethyst_core::specs::{ReadExpect, Read};

use {Loader, AssetStorage, Asset, Handle, Format, Progress};


/// Helper type for loading assets
#[derive(SystemData)]
pub struct AssetLoadHelper<'a, A> where A: Asset {
    loader: ReadExpect<'a, Loader>,
    storage: Read<'a, AssetStorage<A>>
}

impl<'a, A> AssetLoadHelper<'a, A> where A: Asset {
    ...
    pub fn load<F, N, P>(
        ...
        where
            ...
            P: Progress,
    {
        self.loader.load(name, format, options, progress, &*self.storage)
    }
    ...
    pub fn load_from<F, N, P, S>(
        ...
        where
            ...
            String: Borrow<S>,
    {
        self.loader.load_from(name, format, options, source, progress, &*self.storage)
    }
