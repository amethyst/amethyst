...
impl<'a> System<'a> for SpriteVisibilitySortingSystem {
    ...
    fn run(
        &mut self,
        (entities, mut visibility, hidden, hidden_prop, active, camera, transparent, global, screen_spaces): Self::SystemData,
    ) {
        ...
        self.centroids.extend(
            (&*entities, &global, !&hidden, !&hidden_prop, screen_spaces.maybe())
                .join()
                .map(|(entity, global, _, _, screen_space)| (entity, global.0.transform_point(&origin), screen_space))
                .map(|(entity, centroid, screen_space)| (Internals {
                    entity,
                ...
                .filter(|(c, screen_space)| c.from_camera.dot(&camera_backward) < 0. || screen_space.is_some())
                .map(|(c,_)| c),
        );
