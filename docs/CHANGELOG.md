...
* Changed the names of many TransformComponent functions to better reflect their actual function and reduce potential semantic confusion ([#1451])
...
* Added generic parameter type to `TransformComponent` to configure floating point precision (then removed). ([#1334]) ([#1584])
...
* Added a `pivot` field to `UiTransformComponent`. ([#1571])
...
* Renamed all components by adding the word "Component" as suffix. ([#1670]) 
...
* `sprite_camera_follow` example showing how to use a Camera that has a sprite ParentComponent ([#1099])
...
* Added transform constructor function `TransformComponent::new()`. ([#1187])
...
* `TransformComponent::look_at` renamed to `TransformComponent::face_towards` and behavior fixed. ([#1142])
...
* Reordered arguments for `TransformComponent::set_rotation_euler` to match nalgebra's Euler angles. ([#1052])
...
* `OnUiActionImageComponent` and `OnUiActionSound` have been removed as they now work through `EventRetrigger`s ([#1189])
...
* `SpriteRenderComponent` pass to draw sprites without using `Material` and `Mesh`. ([#829], [#830])
...
* Added implementation of From<Vector3<f32>> for TransformComponent which creates a TransformComponent using Vector3 as the translation vector. ([#946])
...
* Added a `HiddenComponent` Component, that hides a single entity, and a HideHierarchySystem that toggles `HiddenComponent` on all children when used. ([#1001])
...
* UI `ScaleMode` is now functional, permitting percentage based `UiTransformComponent`s. ([#774])
...
* UI systems will now never overwrite your local `UiTransformComponent` values ([#774])
* Global `UiTransformComponent` values are no longer writable ([#774])
* `UiResize` refactored to be more user friendly and more helpful ([#774])
* `Anchored` and `Stretched` components have been folded into `UiTransformComponent` ([#774])
...
* `TransformComponent` refactored, and added lots of utility functions ([#660])
