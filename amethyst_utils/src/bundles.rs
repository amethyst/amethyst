...
impl<'a, 'b, A, B> ECSBundle<'a, 'b> for FlyCameraBundle<A,B> where A: Send+Sync+Hash+Eq+Clone+'static, B: Send+Sync+Hash+Eq+Clone+'static{
    fn build(
        self,
        world: &mut World,
        ...
        Ok(
            builder.add(
                FlyCameraMovementSystem::<A,B>::new(1.0, self.move_x_name, self.move_y_name, self.move_z_name),
                "cam_move_system",
                &[],
            )
            .add(
