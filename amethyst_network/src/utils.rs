...
pub fn send_to<T>(event: NetEvent<T>, buf: &mut NetSendBuffer<T>, target: &NetConnection) where T: Send+Sync+'static{
...
pub fn send_to_all<T>(event: NetEvent<T>, buf: &mut NetSendBuffer<T>, pool: &NetConnectionPool) where T: Send+Sync+Clone+'static{
    for conn in &pool.connections{
        send_to(event.clone(),buf,conn);
    }
}

pub fn send_to_all_except<T>(event: NetEvent<T>, buf: &mut NetSendBuffer<T>, pool: &NetConnectionPool, except: &NetConnection) where T: Send+Sync+Clone+'static{
