...
#[cfg(test)]
mod test{
  use amethyst_core::specs::{World,RunNow,VecStorage,ReadStorage,WriteStorage,Join};
  ...
  fn single_packet_early(){
    ...
    let mut conn_to_server = NetConnection::<()>::new(
        SocketAddr::new(IpAddr::from_str("127.0.0.1").unwrap(),21200)
    );
    let mut conn_to_client = NetConnection::<()>::new(
        SocketAddr::new(IpAddr::from_str("127.0.0.1").unwrap(),21201)
    );
    
    //let test_event = NetEvent::TextMessage{msg: "Test Message From Client1".to_string()};
    let test_event = NetEvent::TextMessage{msg: "1".to_string()};
