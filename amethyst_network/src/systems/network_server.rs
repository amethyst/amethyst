...
impl<T> NetworkBase<T> for NetServerSystem<T>
where
    T: Send + Sync + Serialize + Clone + DeserializeOwned + BaseNetEvent<T> + 'static,
{
}

impl<'a, T> System<'a> for NetServerSystem<T>
where
    T: Send + Sync + Serialize + Clone + DeserializeOwned + BaseNetEvent<T> + 'static,
{
                                ...
                                None => {
                                    ...
                                    match T::custom_to_base(ev.clone()) {
                                        Some(NetEvent::Connect) => {
                                            println!(
                                                "Remote ({:?}) initialized connection sequence.",
                                                src
                                            );
                                            ...
                                            self.send_event(
                                                &T::base_to_custom(NetEvent::Connected),
                                                &conn,
                                                &self.socket,
                                            );
