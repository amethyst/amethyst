...
enum InternalSocketEvent<E>{
    SendEvents{target: SocketAddr,events: Vec<NetEvent<E>>},
    Stop,
}
                                  ...
                                  match net_event {
                                    Ok(ev) => {
                                        ...
                                        if !filtered {
                                            ...
                                            info!("Filtered an incoming network packet from source {:?}", raw_event.source);
                                        }
                                    },
                                    Err(e) => error!("Failed to deserialize an incoming network event: {} From source: {:?}", e, raw_event.source),
                                  }
                              }
                          if !matched {
