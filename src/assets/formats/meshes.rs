...
use wavefront_obj::obj::{Normal, NormalIndex, Object, ObjSet, parse, Primitive, TVertex, TextureIndex, Vertex, VertexIndex};
...
fn convert(object: &Object, vi: VertexIndex, ti: Option<TextureIndex>, ni: Option<NormalIndex>) -> PosNormTex {
    PosNormTex {
        a_position: {
...
fn convert_primitive(object: &Object, prim: &Primitive) -> Option<[PosNormTex;3]> {
    match *prim {
        Primitive::Triangle(v1, v2, v3) => {
            Some([convert(object, v1.0, v1.1, v1.2),
...
fn from_data(obj_set: ObjSet) -> Vec<PosNormTex> {
    ...
    let vertices = obj_set.objects.iter().flat_map(|object| {
        object.geometry.iter().flat_map(move |geometry| {
            geometry.shapes.iter().filter_map(move |s| convert_primitive(object, &s.primitive))
        })
    });
