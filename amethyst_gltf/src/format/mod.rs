...
use assets::{Error as AssetError, Format, FormatValue, Prefab, Result as AssetResult, ResultExt,
             Source};
use core::cgmath::{EuclideanSpace, Point3};
...
fn load_scene(
    ...
    let mut bounding_box = Point3::origin()..Point3::origin();
    ...
    // if move to origin is requested shift the whole scene so the center of the bounding box is at
    // the origin
    if options.move_to_origin {
        let center = (bounding_box.start + bounding_box.end.to_vec()) / 2.;
        prefab
            .data_or_default(0)
            .transform
            .as_mut()
...
fn load_node(
    ...
    bounding_box: &mut Range<Point3<f32>>,
) -> Result<(), GltfError> {
    ...
    if let Some(mesh) = node.mesh() {
        ...
        if graphics.len() == 1 {
            ...
            if let Some(bounds) = bounds {
                let pmin = bounds.start;
                let pmax = bounds.end;
                for i in 0..3 {
                    if pmin[i] < bounding_box.start[i] {
                        bounding_box.start[i] = pmin[i];
                    }
                    if pmax[i] > bounding_box.end[i] {
                        bounding_box.end[i] = pmax[i];
                    }
                }
            }
