...
pub fn load_material(
    ...
    prefab.albedo_index = material.index().map(|i| i * 100);
    ...
    prefab.metallic_index = material.index().map(|i| i * 10 + 1);
    prefab.roughness = Some(roughness);
    prefab.roughness_index = material.index().map(|i| i * 10 + 2);
    ...
    prefab.emission_index = material.index().map(|i| i * 10 + 3);
    ...
    prefab.normal_index = material.index().map(|i| i * 10 + 4);
    ...
    prefab.ambient_occlusion_index = material.index().map(|i| i * 10 + 5);
