...
pub fn load_skin<N: Clone + Debug + Default + DeserializeOwned + Serialize + NumCast + RealField + From<f32>>(
    ...
    let inverse_bind_matrices = reader
        ...
        .map(|matrices| matrices.map(|m| [[m[0][0].into(), m[0][1].into(), m[0][2].into(), m[0][3].into()],
                                     [m[1][0].into(), m[1][1].into(), m[1][2].into(), m[1][3].into()],
                                     [m[2][0].into(), m[2][1].into(), m[2][2].into(), m[2][3].into()],
                                     [m[3][0].into(), m[3][1].into(), m[3][2].into(), m[3][3].into()]].into()).collect())
    ...
    let joint_transforms = JointTransformsPrefab::new(
        skin_entity,
        joints.len(),
    );
