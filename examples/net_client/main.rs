...
fn run() -> Result<()> {
    let game = Application::build("", State1)?
        ...
        .with_bundle(
            NetworkBundle::<()>::new("127.0.0.1", Some(3455 as u16), vec![Box::new(FilterConnected::<()>::new())], false)
    ...
    fn run(&mut self, (mut send_buf, pool, time): Self::SystemData) {
        for i in 0..10000{
            let ev = NetEvent::TextMessage {
                msg: format!("CL: frame:{},abs_time:{},c:{}",time.frame_number(), time.absolute_time_seconds(),i),
            };
