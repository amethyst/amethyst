...
lazy_static::lazy_static! {
    ...
    static ref VERTEX: SpirvShader = SpirvShader::from_bytes(
        include_bytes!("./assets/shaders/compiled/vertex/scrolling.vert.spv"),
        ShaderStageFlags::VERTEX,
        "main",
    ).unwrap();

    static ref FRAGMENT: SpirvShader = SpirvShader::from_bytes(
        include_bytes!("./assets/shaders/compiled/fragment/scrolling.frag.spv"),
        ShaderStageFlags::FRAGMENT,
        "main",
    ).unwrap();
}

pub struct ScrollingSprite {
    pub sprite: SpriteRender,
}

impl Component for ScrollingSprite {
...
impl Base2DPassDef for ScrollingSpritePassDef {
    ...
    type SpriteComponent = ScrollingSprite;
...
impl<B: Backend> RenderPlugin<B> for RenderScrollingSprite {
    ...
    fn on_plan(
        ...
        plan.extend_target(self.target, |ctx| {
            ctx.add(
                RenderOrder::Opaque,
                ScrollingSpritePassDesc::new().builder(),
            )?;
            ctx.add(
                RenderOrder::Transparent,
                ScrollingSpritePassTransparentDesc::new().builder(),
            )?;
            Ok(())
        });
        Ok(())
