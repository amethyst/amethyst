...
impl<'a, 'b> SimpleState<'a, 'b> for Example {
    fn on_start(&mut self, data: StateData<GameData>) {
        ...
        println!("Load mesh");
        let (mesh, albedo) = {
            let mesh = world.exec(|loader: AssetLoadHelper<Mesh>| loader.load_from_data(
                Shape::Sphere(32, 32).generate::<Vec<PosNormTangTex>>(None),
                (),
            ));
            let albedo = world.exec(|loader: AssetLoadHelper<Texture>| loader.load_from_data(
        ...
        for i in 0..5 {
            for j in 0..5 {
                ...
                let (metallic, roughness) = world.exec(|loader: AssetLoadHelper<Texture>| (
