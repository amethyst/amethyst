use amethyst::{core::{math::Vector3, transform::{Parent, Transform}}, ecs::{ParallelRunnable, System,IntoQuery}, input::InputHandler, renderer::{ActiveCamera, Camera}, window::ScreenDimensions};
use legion::{Entity, SystemBuilder, component};
    ...
    fn build(&'static mut self) -> Box<dyn ParallelRunnable> {
        Box::new(
            SystemBuilder::new("CameraSwitchSystem")
                ...
                .with_query(<(Entity, &Parent)>::query().filter(component::<Camera>() & component::<Transform>()))
                .build( move |commands, world, (active_camera, dimensions, input), camera_query| {
