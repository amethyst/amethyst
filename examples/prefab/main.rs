...
use amethyst::assets::{AssetStorage, Handle, Loader, PrefabData, PrefabLoader, PrefabLoaderSystem,
                       Result as AssetResult, RonFormat, SimpleFormat};
use amethyst::config::Config;
use amethyst::core::transform::{GlobalTransform, TransformBundle, TransformPrefabData};
use amethyst::ecs::error::Error as SpecsError;
use amethyst::ecs::prelude::{Entity, Read, ReadExpect, World, WriteStorage};
...
use amethyst::renderer::{Camera, DisplayConfig, DrawShaded, Event, KeyboardInput, Light, Material,
                         ...
                         Projection, RenderBundle, Rgba, Stage, Texture, VirtualKeyCode,
                         WindowEvent};
...
struct CustomPrefabData {
    mesh: String,
    material: [f32; 4],
}

type MyPrefabData = (CustomPrefabData, TransformPrefabData);

impl<'a> PrefabData<'a> for CustomPrefabData {
    type SystemData = (
    ...
    fn load_prefab(
        ...
        let mesh = loader.load(self.mesh.as_ref(), Custom, (), (), &meshes);
        let albedo = loader.load_from_data(self.material.into(), (), &textures);
        let mat = Material {
            albedo,
            ..mat_defaults.0.clone()
        };
        mesh_handles.insert(entity, mesh)?;
        materials.insert(entity, mat).map(|_| ())
    }
}
