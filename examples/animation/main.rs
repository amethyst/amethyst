                    ...
                    match virtual_keycode {
                        Some(VirtualKeyCode::Space) => {
                            let animation = world
                                ...
                                .get(self.sphere.unwrap())
                                .and_then(|s| s.get(&self.current_animation))
                                .cloned()
                                .unwrap();
                            let mut sets = world.write();
                            let mut control_set = get_animation_set::<AnimationId, Transform>(
                                &mut sets,
                                self.sphere.unwrap().clone(),
                            );
                            if control_set.has_animation(self.current_animation) {
                                control_set.toggle(self.current_animation);
                            } else {
                        ...
                        Some(VirtualKeyCode::D) => {
                            ...
                            }
                            {
                                let animation = world
                                    .read::<AnimationSet<AnimationId, Transform>>()
                                    .get(self.sphere.unwrap())
                                    .and_then(|s| s.get(&AnimationId::Rotate))
                                    .cloned()
                                    .unwrap();
                                let mut sets = world.write();
                                let mut control_set = get_animation_set::<AnimationId, Transform>(
                            ...
                            }
                            {
                                let animation = world
                                    .read::<AnimationSet<AnimationId, Transform>>()
                                    .get(self.sphere.unwrap())
                                    .and_then(|s| s.get(&AnimationId::Scale))
                                    .cloned()
                                    .unwrap();
                                let mut sets = world.write();
                                let mut control_set = get_animation_set::<AnimationId, Transform>(
                                ...
                                control_set.add_deferred_animation(
                                    ...
                                    AnimationCommand::Start,
                                    AnimationId::Rotate,
                                    DeferStartRelation::Start(0.666),
                                );
                            }
                        }
...
fn initialise_animation(world: &mut World, entity: Entity) {
    ...
    set.insert(
        ...
        loader.load_from_data(
            Animation {
                nodes: vec![
                    (0, TransformChannel::Translation, translation_sampler_handle),
                ],
            },
            (),
            &animation_storage,
    ...
    set.insert(
        ...
        loader.load_from_data(
            Animation {
                nodes: vec![(0, TransformChannel::Scale, scale_sampler_handle)],
            },
            (),
            &animation_storage,
        ),
    ...
    set.insert(
        ...
        loader.load_from_data(
            Animation {
                nodes: vec![(0, TransformChannel::Rotation, rotation_sampler_handle)],
            },
            (),
            &animation_storage,
        ),
