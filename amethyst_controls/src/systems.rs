...
pub fn build_fly_movement_system<T: BindingTypes>(
    speed: f32, 
    right_axis: Option<T::Axis>, 
    up_axis: Option<T::Axis>, 
    forward_axis: Option<T::Axis>,
    ...
    Box::new(move |_world, _resources| {
        SystemBuilder::<()>::new("FlyMovementSystem")
            ...
            .with_query(<(Read<FlyControl>, Read<Rotation>, Write<Translation>)>::query())
            .build(move |commands, world, (time, input), controls| {
                #[cfg(feature = "profiler")]
                profile_scope!("fly_movement_system");

                let x = get_input_axis_simple(&right_axis, &input);
                let y = get_input_axis_simple(&up_axis, &input);
                let z = get_input_axis_simple(&forward_axis, &input);

                if let Some(dir) = Unit::try_new(Vector3::new(x, y, z), convert(1.0e-6)) {
                    for (_, rot, mut trans) in controls.iter_mut(world) {
                        let delta_sec = time.delta_seconds();
                        trans.0 *= Translation3::from(rot.0 * (dir.as_ref() * delta_sec * speed));
                    }
                }
            })
    })
...
//
//            for (control, rot, mut trans) in controls.iter_mut(world) {
